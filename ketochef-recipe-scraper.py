...
class RecipeContentParser(HTMLParser):
    ...
    INGREDIENT_KEYWORDS = {"מצרכים", "הרכיבים", "רכיבים"}
    INSTRUCTION_KEYWORDS = {"אופן ההכנה", "אופן הכנה", "הוראות", "הכנה"}
    ...
    def __init__(self) -> None:
        ...
        self._content_widget_stack: List[bool] = []
    ...
    def handle_starttag(self, tag: str, attrs: List[tuple[str, Optional[str]]]) -> None:
        ...
        widget_attr = attrs_dict.get("data-widget_type", "")
        ...
        if tag in self.HEADING_TAGS:
            ...
            if not self._heading_buffer:
                self._heading_buffer = []
        if tag == "li":
            self._list_item_stack.append(True)
        elif tag == "ul" or tag == "ol":
            self._list_item_stack.append(False)

        parent_widget = self._content_widget_stack[-1] if self._content_widget_stack else False
        is_widget = widget_attr == "theme-post-content.default"
        self._content_widget_stack.append(parent_widget or is_widget)

        current_section = self._section_stack[-1] if self._section_stack else ""
        ...
        self._section_stack.append(current_section)

    def handle_endtag(self, tag: str) -> None:
        if tag == "h1":
            self._in_h1 = False
        if tag in self.HEADING_TAGS and self._heading_stack:
            ...
            if not self._heading_stack:
                ...
                if normalized in self.INGREDIENT_KEYWORDS:
                    self._current_section = "ingredients"
                elif normalized in self.INSTRUCTION_KEYWORDS:
                    self._current_section = "instructions"
                self._heading_buffer = []
        ...
        if self._content_widget_stack:
            self._content_widget_stack.pop()
    ...
    def handle_data(self, data: str) -> None:
        ...
        in_widget = self._content_widget_stack[-1] if self._content_widget_stack else False
        ...
        if not section and self._current_section:
            section = self._current_section
        in_list_item = self._list_item_stack[-1] if self._list_item_stack else False
        if in_widget and section and in_list_item:
